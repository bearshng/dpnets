...
class AddNoiseDynamic(object):
    ...
    def __call__(self, img):
        ...
        for z in range(img.shape[0]):
            noise[z,:,:]=np.random.randn(*img[z,:,:].shape)* random.random()*self.sigma_ratio
        # print(img.sum(), noise.sum())
        return img + noise


class AddNoiseDynamicList(object):
    """add gaussian noise to the given numpy array (B,H,W)"""
    def __init__(self, sigma):
        sigma =np.array(sigma)
        self.sigma_ratio_list = sigma / 255.        
    
    def __call__(self, img):
        #noise = np.random.randn(*img.shape) * self.sigma_ratio
        noise = np.random.randn(*img.shape)
        sigma_ratio = random.choice(self.sigma_ratio_list)
        for z in range(img.shape[0]):
            noise[z,:,:]=np.random.randn(*img[z,:,:].shape)* random.random()*sigma_ratio
        # print(img.sum(), noise.sum())
        return img + noise
