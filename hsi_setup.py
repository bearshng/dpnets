...
def train_options(parser):
    ...
    parser.add_argument('--num_half_layer', type=int, default=3)
    parser.add_argument('--unfolding', type=int, default=3)
...
class Engine(object):
    ...
    def __setup(self):
        ...
        from models.DPNetU import Params
        params = Params(in_channels=1, channels=self.opt.channels,
                         num_half_layer=self.opt.num_half_layer,unfolding=self.opt.unfolding,bn=self.opt.bn, conv_num=self.opt.conv_num)
    ...
    def validate(self, valid_loader, name):
        ...
        ###modified###
                    ...
                    #scio.savemat(fname+'Res.mat', {'outputs': outputs.cpu().detach().numpy()})      
